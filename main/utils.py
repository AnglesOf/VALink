...
model_output_path = r''  # your saved model path
data_dir = r''  # your path to dataset dir

# model path
text_model_path = r""  # your path to text model
code_model_path = r""  # your path to code model
...
def convert_examples_to_features_noProcess(row):
    return InputFeatures(row['combined_token'], row['combined_ids'], row['label'], row['Issue_KEY'], row['Commit_SHA'])


class TextDataset(Dataset):
    def __init__(self, tokenizer, codeTokenizer, args, file_path=None):
        self.text_examples = []
        self.code_examples = []
        ...
        for i_row, row in df_link.iterrows():
            ...
            self.code_examples.append(convert_examples_to_features(row, codeTokenizer, 'Commit_Code', args))
